...
# ================== FLAGS ==================
FORCE_PLACEHOLDERS = True  # keep AI off for now; flip later
...
st.markdown(
    ...
    <style>
      ...
      /* Global page frame (outside the component) */
      .global-frame {
...
def load_syllabus() -> Dict:
    # Expect a syllabus.json in the repo root. Example structure:
    # { "Biology": { "Module 1": { "IQ A": ["Dotpoint 1", "Dotpoint 2"] } } }
...
# ================== COMPONENT WRAPPER ==================
# Custom DnD component is prebuilt under frontend/build
...
def dnd_cloze(
    segments: List[str],
    answers: List[str],
    initial_bank: List[str],
    initial_fills: List[str | None],
    *,
    ...
    """Invoke the custom DnD cloze component.
    Returns dict: {"bank":[...], "fills":[...]} (or None on first render)
    """
    return _dnd_cloze(
        segments=segments,
        answers=answers,
...
def smart_fp(dotpoint: str, subject: str) -> str:
    # Better non-AI FP prompt template by subject
...
def placeholder_report(ans: str) -> Dict:
    # Minimal heuristic list suggestions (AI disabled)
    return {
...
def placeholder_cloze(level:int=0) -> str:
    # Longer multi-sentence clozes with multiple blanks
    if level == 0:
        return (
    ...
    # More specific/targeted variant
    return (
...
def fp_followup_questions(weakness: str, subject: str, dotpoint: str) -> List[str]:
    # Two tailored follow-ups per weakness
    if subject.lower() in ("physics","chemistry"):
        return [
...
if st.session_state.nav_guard:
    ...
    with colg[0]:
        if st.button("Save as incomplete (SRS next)", use_container_width=True):
            # TODO: persist incomplete + SRS priority
            reset_flow()
...
# ----- CLOZE (DnD, review persists until Continue) -----
    ...
    target_label = (
        st.session_state.weak_list[st.session_state.weak_index]
        if st.session_state.weak_list and st.session_state.weak_index < len(st.session_state.weak_list)
        else "this topic"
    )
    ...
    # Global page frame drawn from Streamlit side (so it overlays whole app)
    if st.session_state.correct_flags is not None:
        c = sum(st.session_state.correct_flags)
        total = len(st.session_state.correct_flags) or 1
        bad_pct = int((total - c) / total * 100)
        ...
        st.markdown(
            ...
            f'<strong>Review your cloze:</strong> Score {c}/{total}. Green/red borders show correct/incorrect per blank. '
            f'The full-page border matches overall correctness.'
            f'</div>',
            unsafe_allow_html=True
        )
...
# ----- POST-CLOZE (rating + add weaknesses; branch to extra cloze or FP follow-ups) -----
    ...
    if go:
        ...
        if rating <= 6:
            # one more specific cloze on the same weakness, then FP
            ...
            # go to FP follow-ups for this weakness
            st.session_state.cloze_specificity = 0
            st.session_state.current_cloze = None
            st.session_state.correct_flags = None
            st.session_state._dnd_segs = None
...
# ----- FP FOLLOW-UPS (tailored to current weakness) -----
    ...
    if st.session_state.weak_list and st.session_state.weak_index < len(st.session_state.weak_list):
        ...
        current_weak = "this topic"

    st.markdown(f"**Follow-up first-principles questions for:** _{current_weak}_")
    qlist = fp_followup_questions(current_weak, subject, dotpoint)

    with st.form(key=f"fp_follow_{st.session_state.weak_index}"):
        a1 = st.text_area("Q1: " + qlist[0], height=160)
        a2 = st.text_area("Q2: " + qlist[1], height=160)
        submitted = st.form_submit_button("Next")
    if submitted:
        # (Optional: store a1, a2 for review/logging later)
        ...
        if st.session_state.weak_index < len(st.session_state.weak_list):
            # next weakness → new cloze (broad) → rating → FP, etc.
            st.session_state.stage = "cloze"
        else:
            st.success("Weakness cycle complete. Next step: overall dotpoint rating / exam mode (to be added).")
            st.session_state.stage = "fp"
        st.rerun()
